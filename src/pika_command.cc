...
void Cmd::DoCommand(const HintKeys& hint_keys) {
  // 如果需要缓存处理，且缓存模式不为 PIKA_CACHE_NONE 且缓存状态正常
  if (IsNeedCacheDo() && PIKA_CACHE_NONE != g_pika_conf->cache_mode() &&
    ...
    // 如果是读操作且缓存丢失，或缓存丢失标记为 rtc，执行数据库操作
  ...
  // 如果是非管理员命令且执行成功，统计命令的命中情况
  if (!IsAdmin() && res().ok()) {
    if (res().noexist()) {
      g_pika_server->incr_server_keyspace_misses();  // 如果键值不存在，增加命中失效次数
    } else {
      g_pika_server->incr_server_keyspace_hits();  // 如果键值存在，增加命中次数
    }
  }
}
...
bool Cmd::DoReadCommandInCache() {
  ...
  if (db_->cache()->CacheStatus() == PIKA_CACHE_STATUS_OK) {
    ...
    // 如果是读命令且缓存未命中，返回 true
    if (is_read() && !res().CacheMiss()) {
      return true;
    }
  }

  return false;  // 如果缓存未命中或缓存不可用，返回 false
}

// 处理 binlog
void Cmd::DoBinlog() {
  // 如果命令执行成功且是写操作，并且配置开启了写入binlog
  if (res().ok() && is_write() && g_pika_conf->write_binlog()) {
    std::shared_ptr<net::NetConn> conn_ptr = GetConn();  // 获取连接对象
    std::shared_ptr<std::string> resp_ptr = GetResp();   // 获取响应对象

    // 如果命令是系统添加的虚拟命令，连接和响应对象可能为空
    // 如果是虚拟命令且连接或响应为空，直接返回错误
    if ((!conn_ptr || !resp_ptr) && (name_ != kCmdDummy)) {
      if (!conn_ptr) {
        LOG(WARNING) << sync_db_->SyncDBInfo().ToString() << " conn empty.";  // 连接为空警告
      }
      if (!resp_ptr) {
        LOG(WARNING) << sync_db_->SyncDBInfo().ToString() << " resp empty.";  // 响应为空警告
      }
      res().SetRes(CmdRes::kErrOther);  // 设置错误响应
      return;
    }
