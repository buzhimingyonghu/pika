...
void SlaveofCmd::DoInitial() {
  ...
  LOG(WARNING) << "SlaveofCmd::master_ip_: " << master_ip_;
  LOG(WARNING) << "SlaveofCmd::argv_[1]: " << argv_[1];
  master_ip_ = argv_[1];
...
void DbSlaveofCmd::Do() {
  ...
  if (is_none_) {
    ...
    if (slave_db->State() == ReplState::kNoConnect || slave_db->State() == ReplState::kError ||
      ...
      s = g_pika_rm->ActivateSyncSlaveDB(RmNode(g_pika_server->master_ip(), g_pika_server->master_port(), db_name_, 0),
                                         state);
    }
  }
...
void FlushallCmd::Do() {
  ...
  if (flushall_succeed_) {
    ...
    res_.SetRes(CmdRes::kErrOther,
                "Flushall failed, maybe only some of the dbs successfully flushed while some not, check WARNING/ERROR "
                "log to know "
                "more, you can try again moment later");
  }
}

void FlushallCmd::DoThroughDB() { Do(); }
...
bool FlushallCmd::DoWithoutLock(std::shared_ptr<DB> db) {
  ...
  if (!success) {
    ...
    res_.SetRes(CmdRes::kErrOther,
                db->GetDBName() + " flushall failed due to other Errors, please check Error/Warning log to know more");
    return false;
  }
  DoFlushCache(db);
  return true;
...
void FlushdbCmd::Do() {
  ...
  if (flush_succeed_) {
    ...
    res_.SetRes(CmdRes::kErrOther,
                "flushdb failed, maybe you cna try again later(check WARNING/ERROR log to know more)");
  }
}

void FlushdbCmd::DoThroughDB() { Do(); }
...
void ClientCmd::DoInitial() {
  ...
  if ((strcasecmp(argv_[1].data(), "list") == 0) && argv_.size() == 2) {
    ...
    info_ = argv_[2];
  } else if (argv_.size() == 4 && (strcasecmp(argv_[1].data(), "kill") == 0) &&
             (strcasecmp(argv_[2].data(), "type") == 0) &&
             ((strcasecmp(argv_[3].data(), KILLTYPE_NORMAL.data()) == 0) ||
              (strcasecmp(argv_[3].data(), KILLTYPE_PUBSUB.data()) == 0))) {
...
void InfoCmd::InfoReplication(std::string& info) {
  ...
  for (const auto& db_item : g_pika_server->GetDB()) {
    std::shared_ptr<SyncSlaveDB> slave_db = g_pika_rm->GetSyncSlaveDBByName(DBInfo(db_item.second->GetDBName()));
    if (!slave_db) {
...
void ConfigCmd::ConfigSet(std::shared_ptr<DB> db) {
  ...
  if (set_item == "timeout") {
    ...
    if (pstd::string2int(value.data(), value.size(), &ival) == 0) {
      res_.AppendStringRaw("-ERR Invalid argument \'" + value +
                           "\' for CONFIG SET 'min-write-buffer-number-to-merge'\r\n");
      return;
    }
    ...
    if (pstd::string2int(value.data(), value.size(), &ival) == 0) {
      res_.AppendStringRaw("-ERR Invalid argument \'" + value +
                           "\' for CONFIG SET 'level0-slowdown-writes-trigger'\r\n");
      return;
    }
    ...
    if (pstd::string2int(value.data(), value.size(), &ival) == 0) {
      res_.AppendStringRaw("-ERR Invalid argument \'" + value +
                           "\' for CONFIG SET 'level0-file-num-compaction-trigger'\r\n");
      return;
    }
    ...
    if (pstd::string2int(value.data(), value.size(), &ival) == 0 || ival < PIKA_MAX_CONN_RBUF_LB ||
        ival > PIKA_MAX_CONN_RBUF_HB * 2) {
...
void LastsaveCmd::Do() { res_.AppendInteger(g_pika_server->GetLastSave()); }
...
void PKPatternMatchDelCmd::DoInitial() {
  ...
  if (argv_.size() > 2) {
    if (pstd::string2int(argv_[2].data(), argv_[2].size(), &max_count_) == 0 || max_count_ < 1 ||
        max_count_ > storage::BATCH_DELETE_LIMIT) {
      res_.SetRes(CmdRes::kInvalidInt);
      return;
    }
...
void PKPatternMatchDelCmd::DoThroughDB() { Do(); }
