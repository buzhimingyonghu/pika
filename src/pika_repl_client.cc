...
PikaReplClient::~PikaReplClient() {
  // 函数作用：析构函数，停止客户端线程并输出日志。
  // 功能：在销毁 `PikaReplClient` 对象时，停止客户端线程并打印日志。

  // 停止客户端线程
  client_thread_->StopThread();

  // 输出日志，表示 `PikaReplClient` 退出
  LOG(INFO) << "PikaReplClient exit!!!";
}

int PikaReplClient::Start() {
  // 函数作用：启动客户端线程以及写 binlog 和写 DB 的工作线程。
  // 功能：启动客户端线程以及所有的写 binlog 和写 DB 的工作线程。如果启动失败，记录错误日志。
...
int PikaReplClient::Stop() {
  // 函数作用：停止所有工作线程并确保所有写 DB 任务完成后再退出。
  // 功能：停止客户端线程和所有写 binlog 及写 DB 的工作线程，确保所有的异步写 DB 任务完成后才退出。
  ...
  // 异步写 DB 任务必须全部完成后才退出，防止数据丢失
  bool all_write_db_task_done = true;
  do {
    // 检查所有写 DB 任务的队列大小
    for (auto& db_worker : write_db_workers_) {
      if (db_worker->TaskQueueSize() != 0) {
        // 如果存在未完成的写 DB 任务，继续等待
        all_write_db_task_done = false;
        std::this_thread::sleep_for(std::chrono::microseconds(300));  // 睡眠 300 微秒后继续检查
        break;
      } else {
        // 如果所有写 DB 任务完成，设置为 true
        all_write_db_task_done = true;
      }
    }
    // 如果还有未完成的写 DB 任务，继续等待
...
void PikaReplClient::Schedule(net::TaskFunc func, void* arg) {
  // 函数作用：调度任务，将任务分配给下一个可用的 binlog 写工作线程。
  // 功能：根据当前的调度顺序，将任务分配给下一个可用的 binlog 写工作线程。

  // 将任务调度给下一个可用的 binlog 写工作线程
  write_binlog_workers_[next_avail_]->Schedule(func, arg);

  // 更新下一个可用的 binlog 写工作线程的索引
  UpdateNextAvail();
}
